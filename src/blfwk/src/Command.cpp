...
{
#if FILESYSTEM_ALT
    m_filePointer = fopen_alt(filePath.c_str(), "rb");
#else
    ...
    {
#if FILESYSTEM_ALT
        ::fseek_alt(m_filePointer, 0, SEEK_END);
        m_fileSize = ftell_alt(m_filePointer);
        ::fseek_alt(m_filePointer, 0, SEEK_SET);
#else
...
#if FILESYSTEM_ALT
    return (uint32_t)fread_alt(data, 1, size, m_filePointer);
#else
...
{
#if FILESYSTEM_ALT
    m_filePointer = fopen_alt(filePath.c_str(), "wb");
#else
...
{
#if FILESYSTEM_ALT
    fwrite_alt(data, 1, size, m_filePointer);
#else
    ...
    while (m_dataProducer->hasMoreData() && *bytesWritten < m_dataProducer->getDataSize())
    {
        ...
        if (count)
        {
            ...
            *bytesWritten += count;
            ...
            /* The process takes ~5ms between data packets and updates download progress, Remove the process to accelerate the download speed*/
            if (progress != NULL)
            {
