...
enum _timing_constants
{
    ...
    kDefaultPingDelayUs = 100,            /* Host sends 0x5AA6 -> TIMEOUT -> Host reads 0x5A */
    kDefaultByteB2BWriteTimeoutUs = 1000, /* Host sends target -> TIMEOUT -> Host sends target again */
    kDefaultByteWriteCmdTimeoutUs = 100,  /* Host sends command to target -> TIMEOUT -> Host reads response from target */
    kDefaultByteWriteDataTimeoutUs = 100, /* Host sends data to target -> TIMEOUT -> Host reads response from target */
    kDefaultByteReadTimeoutUs = 100, //!< Default value for receiving 1 byte timeout
...
enum _serial_packet_constants
{
    ...
    kIncomingPacketBufferSize = kMaxHostPacketSize,
    kOutgoingPacketBufferSize = kMaxHostPacketSize
#else
